...
![LegalLens Interface](interface.png)
...
└──────┬──────┘
       ...
       ▼
┌─────────────────┐
│ Clause Extractor │  ← LLM via Ollama/Groq/Gemini, PydanticOutputParser
└──────┬──────────┘
       │ list[ExtractedClause]
       ▼
┌─────────────────┐
│ Risk Classifier  │  ← chain-of-thought, Low/Medium/High + 0–1 score
└──────┬──────────┘
       │ list[ClassifiedClause]
       ▼
┌──────────────────┐
│ Precedent        │  ← hybrid RAG: ChromaDB (dense) + BM25 (sparse)
│ Retriever        │    runs after classifier (needs risk_level)
└──────┬───────────┘
       │ dict[clause_id → list[Precedent]]
       ▼
┌───────────────────────────┐
│  Recommendation Generator  │  ← skips LOW risk, LLM for MEDIUM/HIGH
└──────────────┬────────────┘
               │ list[Recommendation]
               ▼
      ┌─────────────────┐
      │  Report Assembler│  ← PDF + HTML output
      └─────────────────┘
```
